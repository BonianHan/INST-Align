#!/usr/bin/env python
"""Run baseline methods only for the spatial alignment experiment.

This script runs raw/no-align plus selected external methods, and does not
run INST-Align.
"""
...
def main(
    ...
    output_name: str = "baseline_results",
) -> None:
    if config is None:
    ...
    if dlpfc_datasets:
        ...
        if sample_indices:
            print("\n" + "#" * 60)
            print(f"# DLPFC baselines ({len(sample_indices)} sample groups)")
            print("#" * 60)
            ...
            df, *_ = run_spatial_alignment_all(
                ...
                run_stalign=run_stalign, run_insta=False,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run baselines only for Experiment 1")
